package cumcm2018bQ1;

import java.util.ArrayList;
import java.util.List;
public class RGV {//所有时间采用“秒”单位
    public static int CurrentTime;//-TODO 每次操作需要更新
    public static int RGVposition;//0~3，//-TODO 每次移动操作需要更新
    public static List<CNC> SignialQueue;//-TODO 每次操作需要更新
    public static List<MyMaterial> Upmaterials;//使用添加MyMaterial并执行其构造函数表示该物料处于加工中 -TODO 每次操作需要更新
    public static List<MyMaterial> Downmaterials;//调用完downThis后添加进去，用于记录
    ...
    public static void initDatas(){
        ...
        for (int i = 0; i < 8; ++i) {
            SignialQueue.add(new CNC(i,RGVposition,fastT,slowT,workT));
        }
        Upmaterials=new ArrayList<>();
        Downmaterials=new ArrayList<>();
    }
    public static void Assignment(int MaxTime){//位置贪心，CNC贪心
        int opsCount=0;
        while (CurrentTime<=MaxTime){//每次循环RGV进行一次操作并进行更新，
            ...
            int nearPos= findNearestPos();
            System.out.println("最近位置："+(nearPos)+"  当前位置："+RGVposition);
            if (RGVposition==nearPos){
                //选择该位置上时间短的处理
                if (SignialQueue.get(RGVposition*2).addressTime<=SignialQueue.get(RGVposition*2+1).addressTime){//同位置快CNC等待时间短
                    if (SignialQueue.get(RGVposition*2).addressTime>fastT){//该CNC还在加工，需执行等待操作
                        update(SignialQueue.get(RGVposition*2).addressTime-fastT,-1);
                    }else {//上下料
                        int opCNCtype=SignialQueue.get(RGVposition*2).type;
                        if (opCNCtype==1){//还未上过料，只上料
                            SignialQueue.get(RGVposition*2).upLoad(CurrentTime,false);
//                            CurrentTime+=28;
                            update(fastT,-1);
                            continue;
                        }else if (opCNCtype==0){//下料，且downLoad方法已经有了上料过程
                            SignialQueue.get(RGVposition*2).downLoad(CurrentTime);
//                            CurrentTime+=28;
                            callClean=true;
                            update(fastT,-1);
                            continue;
                    ...
                    if (SignialQueue.get(RGVposition*2+1).addressTime>slowT){//需执行等待操作
                        update(SignialQueue.get(RGVposition*2+1).addressTime-slowT,-1);
                    }else {//上下料
                        int opCNCtype=SignialQueue.get(RGVposition*2+1).type;
                        if (opCNCtype==1){//还未上过料，只上料
                            SignialQueue.get(RGVposition*2+1).upLoad(CurrentTime,false);
//                            CurrentTime+=31;
                            update(slowT,-1);
                            continue;
                        }else if (opCNCtype==0){//下料，且downLoad方法已经有了上料过程
                            SignialQueue.get(RGVposition*2+1).downLoad(CurrentTime);
//                            CurrentTime+=31;
                            callClean=true;
                            update(slowT,-1);
                            continue;
                ...
                int step=Math.abs(nearPos-RGVposition);
                int stepTime=0;
                if (step==1){
                    stepTime=stepT1;
                }else if (step==2){
...
//                            CurrentTime+=31;
    ...
    public static int findNearestPos(){
        int[] posRemainTime=new int[4];
        for (int i=0;i<SignialQueue.size();++i){
            posRemainTime[i/2]+=SignialQueue.get(i).addressTime;
        }
        int minValue=Integer.MAX_VALUE;
        for (int i=0;i<4;++i){
            minValue=Math.min(minValue,posRemainTime[i]);
        }
        int i;
        for (i=0;i<4;++i){
            if (posRemainTime[i]==minValue){
                break;
            }
        }
        return i;
