...
public class CNC {//0~7号

    public CNC(int CNCnum, int RGVposition,int fastTime,int slowTime,int workTime) {
        this.CNCnum = CNCnum;
        ...
        if (step == 0) {
            ...
            this.addressTime = RGV.stepT1+udTime;
        } else if (step == 2) {
            this.addressTime = RGV.stepT2+udTime;
        } else if (step == 3) {
            this.addressTime = RGV.stepT3+udTime;
        }else {
            System.err.println("cumcm2018bQ1.CNC：步长计算出错,程序设计出错");
        }
    }
    public void update(int preTime,int newTime,int NewPos){//注意，该方法传入的是RGV完成该操作后的位置
        if (type==1 ||type==2){
            ...
            if (step == 0) {
                ...
                addressTime = RGV.stepT1+udTime;
            } else if (step == 2) {
                addressTime = RGV.stepT2+udTime;
            } else if (step == 3) {
                addressTime = RGV.stepT3+udTime;
            }else {
                System.err.println("cumcm2018bQ1.CNC：步长计算出错,程序设计出错");
            }
...
//                type=2;
    ...
    public void upLoad(int newTime,Boolean isNeedClean){//传入时间为该操作刚开始时间
        ...
        mm=new MyMaterial(newTime,CNCnum);
        RGV.Upmaterials.add(mm);
        type=0;
    ...
    public void downLoad(int newTime){//传入时间为该操作刚开始时间
        ...
        if (mm.downTime+udTime+25<=8*3600) {
            RGV.Upmaterials.remove(mm);
            RGV.Downmaterials.add(mm);
        }
        upLoad(newTime,true);
    }
